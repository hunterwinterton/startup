...
Incorporates many common uses of css and html to help a website look better.

Demo: https://codepen.io/leesjensen/pen/JjZavjW

To add Bootstrap, add the following code in the ```<head>``` of your html file, below the ```<meta>```:
```html
    <link
...
To add the JavaScript components of Bootstrap, add the following code at the very bottom of the ```<body>```:
```html
  <script
